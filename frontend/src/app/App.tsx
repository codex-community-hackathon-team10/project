...
export function App() {
  ...
  if (authState === "loading") return <main className="auth-page"><section className="auth-card"><p className="eyebrow">CAMPUS MATE</p><h2>로그인 상태를 확인하고 있어요…</h2></section></main>;
...
function ServiceApp({ onSignOut }: { onSignOut: () => void }) {
  ...
  return <><header className="site-header"><button className="brand" onClick={() => setTab("matches")} aria-label="Campus Mate 홈"><span>CM</span>Campus Mate</button><nav aria-label="주 메뉴">{tabs.map((item) => <button className={tab === item.id ? "active" : ""} key={item.id} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav><div className="user-menu"><ShibaAvatar className="user-avatar" /><span className="user-name">{profile?.nickname ?? "Campus Mate"}</span><button className="signout" onClick={onSignOut}>로그아웃</button></div></header><main className={isChatMode ? "chat-page" : "app-shell"}>
    ...
    {tab === "profile" && <><FullProfileEditor onSaved={setProfile} /><ProfileView /></>}
  </main></>;
}
...
function ProfileOnboarding({ onComplete }: { onComplete: (profile: Profile) => void }) {
  return <main className="onboarding-page"><section className="onboarding-card"><p className="eyebrow">WELCOME TO CAMPUS MATE</p><h1>점심 메이트를 만나기 전,<br />프로필을 완성해 주세요 🐕</h1><p className="subtle">입력한 정보는 공강이 겹치는 같은 캠퍼스 메이트를 추천하는 데만 사용돼요.</p><FullProfileEditor isOnboarding onSaved={onComplete} /></section></main>;
}

function FullProfileEditor({ isOnboarding = false, onSaved }: { isOnboarding?: boolean; onSaved?: (profile: Profile) => void }) {
