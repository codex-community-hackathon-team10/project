...
export function App() {
  ...
  return <main className="app-shell">
    <header className="hero"><div><p className="eyebrow">LUNCH MATE</p><h1>공강을 같이 보낼<br />친구를 찾아보세요.</h1><p className="subtle">같은 캠퍼스에서 실제로 만날 수 있는 점심 시간을 연결합니다.</p></div><div className="status-pill"><span>●</span> 매칭 준비 완료</div></header>
...
function ScheduleView() {
  ...
  return <section><div className="section-heading"><div><p className="eyebrow">온보딩 2/3</p><h2>시간표와 공강</h2></div><span className="service-window">점심 서비스 11:00–15:00</span></div><div className="schedule-grid">{(Object.keys(dayLabels) as DayOfWeek[]).map((day) => <article className="day-column" key={day}><h3>{dayLabels[day]}요일</h3>{schedules.filter((item) => item.day === day).map((item) => <div className="class-block" key={item.id}><strong>{item.subject}</strong><span>{item.start}–{item.end}</span></div>)}<p className="free-slot">공강: 자동 계산</p></article>)}</div><div className="add-form"><h3>수업 추가</h3><input placeholder="과목명" value={form.subject} onChange={(event) => setForm({ ...form, subject: event.target.value })} /><select value={form.day} onChange={(event) => setForm({ ...form, day: event.target.value as DayOfWeek })}>{(Object.keys(dayLabels) as DayOfWeek[]).map((day) => <option key={day} value={day}>{dayLabels[day]}요일</option>)}</select><Time value={form.start} onChange={(start) => setForm({ ...form, start })} /><Time value={form.end} onChange={(end) => setForm({ ...form, end })} /><button className="primary" onClick={add}>추가</button></div>{message && <p className="form-message">{message}</p>}</section>;
}
