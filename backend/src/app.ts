...
import { createConfiguredRecommendationAi, type RecommendationAi } from "./ai-recommendation-service.js";
...
import { createMatchConversationRouter } from "./http/match-conversation-routes.js";
...
export function createApp(options: { store?: SocialStore; tokenVerifier?: TokenVerifier; coreQueryPort?: CoreQueryPort; recommendationAi?: RecommendationAi; clock?: () => Date } = {}) {
  const app = express();
  ...
  const recommendationAi = options.recommendationAi ?? createConfiguredRecommendationAi();
  const clock = options.clock ?? (() => new Date());
  app.use("/api/v1", requireAuth(options.tokenVerifier ?? createConfiguredTokenVerifier()), createProfileRouter(store), createScheduleRouter(store), createMatchRouter(coreQueryPort, clock), createMatchConversationRouter(store, coreQueryPort, recommendationAi, clock), createVenueRouter(store, clock, recommendationAi), createProposalRouter(store, coreQueryPort, clock));
