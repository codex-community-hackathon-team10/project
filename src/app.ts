...
import { DemoTokenVerifier, requireAuth, type TokenVerifier } from "./http/auth.js";
...
export function createApp(options: { store?: MemoryStore; tokenVerifier?: TokenVerifier } = {}) {
  ...
  app.use("/api/v1", requireAuth(options.tokenVerifier ?? new DemoTokenVerifier()), createProfileRouter(store), createScheduleRouter(store));
